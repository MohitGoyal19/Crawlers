...
def get_data(url, output):
    ...
    specifications = ' | '.join([spec.text.strip().replace('\n', ' ') for spec in page.find('div', {'class': 'detail__specification'}).find_all('div', {'class': 'str'})])
    ...
    for model in range(len(page.find_all('div', {'class': 'model-title'}))):
        ...
        for row in rows:
            ...
            output.loc[x] = {
                ...
                'Years': row.find_all('td')[0].text.strip(),
